...
public class EditMethodsActivity extends Activity implements SourceCodeParseListener {
    ...
    private void updateUI() {
        ...
        boolean haveIssues = !allIssues.isEmpty();

        List<MethodMetadata> methodMetadata = !SourceCode.getClassMetadata().isEmpty() ? SourceCode.getClassMetadata().get(0).getMethodMetadata() : new ArrayList<MethodMetadata>();
    ...
    public void sourceCodeChanged(SourceCodeStatus sourceCodeStatus) {
        if (sourceCodeStatus == SourceCodeStatus.ParsingCompleted) {
            if (SourceCode.getSyntaxErrors().isEmpty()) {
                finish();
            } else {
                updateUI();
